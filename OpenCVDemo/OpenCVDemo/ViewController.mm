...
//  Copyright © 2015年 158954945. All rights reserved.
//
...
@property (weak, nonatomic) IBOutlet UIImageView *canyImageView;
@property (weak, nonatomic) IBOutlet UIImageView *lineImageView;


@property (strong,nonatomic) UIImagePickerController *imgPickerControll;

@property (weak, nonatomic) IBOutlet UIBarButtonItem *loadItem;
...
- (void)reloadImage {
    ...
    Mat img = [Utility cvMatFromUIImage:self.srcImageView.image];
    self.dstImageView.image = [Utility UIImageFromCVMat:scan(img,0)];
    self.canyImageView.image = [Utility UIImageFromCVMat:scan(img, 1)];
    self.lineImageView.image = [Utility UIImageFromCVMat:scan(img, 2)];
}

- (IBAction)clickRightBtn:(UIBarButtonItem *)sender {
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    _imgPickerControll.sourceType =buttonIndex == 0 ?UIImagePickerControllerSourceTypeCamera: UIImagePickerControllerSourceTypePhotoLibrary;
#else
    _imgPickerControll.sourceType = UIImagePickerControllerSourceTypePhotoLibrary;
#endif
    [self presentViewController:self.imgPickerControll animated:YES completion:nil];
}
...
cv::Mat scan(Mat file,int type, bool debug = true) {
    ...
    Mat gray, canny;
    cvtColor(img_proc, gray, CV_BGR2GRAY); //灰度化处理
    ...
    for (size_t i = 0; i < lines.size(); i++) {
        ...
        if (fabs(delta_x) > fabs(delta_y)) {
            ...
            if (debug)
                line(img_proc, cv::Point(v[0], v[1]), cv::Point(v[2], v[3]), Scalar(0, 0, 255),3);
        } else {
            verticals.push_back(l);
            if (debug)
                line(img_proc, cv::Point(v[0], v[1]), cv::Point(v[2], v[3]), Scalar(0, 255, 0),3);
        }
    }
    if (type == 3) {
        return img_proc;
